...
uses
  Classes, SysUtils,
  GLZTypes;//, GLZClasses, GLEPersistentClasses;
...
Type
  ...
  generic TGLZBaseArray<T> = class //(TGLZPersistenObject)
  private
    F: boolean;
    FTagString : string;
...
begin
  {$ifdef DEBUG}
    Assert((FCount>0) and (FPosition<FCount));
  {$endif}
...
begin
  {$ifdef DEBUG}
    Assert((FCount>0) and (FRows>0) and (Position<FCols) and (Position<FCount));
  {$endif}
